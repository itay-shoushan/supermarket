...
import { addProductToCartService, getCartByUserIDService, getCartDetailsByIDService, getCartPriceService, isCartExist, orderCartService, removeProductFromCartService, updateQuantityService } from '../services/carts';
...
export async function getCartByUserIDHandler(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (!cart) return res.status(400).json({ message: "no open carts available to the certain id" });
        else return res.status(200).json({ message: "ok", cart })
    } catch (error) {
...
export async function addProductToCartByIDHandler(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (!result) return res.status(400).json({ message: "porduct failed to add" });
...
export async function updateProductQuantityHandler(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (!result) return res.status(400).json({ message: "update quantity failed" });
